...
这是一个面向后端系统的机器遗忘证据服务，用于记录删除请求、执行状态和可验证回执。当前提供可运行的 Python 包、健康检查入口、删除请求的受理与查询 HTTP API（SQLite 持久化）、请求存储层上的可持久化状态机（状态推进与状态查询）、存储层上的删除执行编排（领取-租约-完成与执行记录），以及存储层上的删除回执生成、核验与可恢复密钥轮换。状态推进、状态查询、执行编排、回执与密钥轮换能力只在存储层开放，HTTP 仍只提供受理与受理回执查询两个端点，不新增任何执行、回执或密钥轮换相关 HTTP 入口。
