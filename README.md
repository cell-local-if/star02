...
这是一个面向后端系统的机器遗忘证据服务，用于记录删除请求、执行状态和可验证回执。当前提供可运行的 Python 包、健康检查入口、删除请求的受理与查询 HTTP API（SQLite 持久化）、请求存储层上的可持久化状态机（状态推进与状态查询）、存储层上的删除执行编排（领取-租约-完成与执行记录），以及存储层上的删除回执生成、核验与可恢复的回执密钥轮换。状态推进、状态查询、执行编排、回执与密钥轮换能力只在存储层开放，HTTP 仍只提供受理与受理回执查询两个端点，不新增任何执行、回执或轮换相关 HTTP 入口。
...
- `generate_receipt(tenant_id, request_id, key)`：为已完成删除且执行记录落定的请求生成外部可核验的删除回执。只有状态为 `completed` 且最早终态尝试记录为 `completed` 的请求可取得回执；`accepted`、`processing`、`failed` 或无落定执行记录的请求统一抛 `ReceiptUnavailable`。多个终态尝试存在时回执关联最早完成的执行结果，后续重复终态不改变回执内容。
  ...
  - 首次呈递的密钥登记为该租户的第 1 代回执密钥；此后只有当前（现用）代次的密钥能生成新回执，旧代或未知密钥生成新回执时抛 `ReceiptKeyConflict`，但已发出的回执始终可用其原密钥核验。
- `verify_receipt(receipt_text, key)`：核验回执文本与密钥。仅当文本格式合法、与已落定回执记录逐字段完全一致、且认证标签在所给密钥下重算一致时返回 `True`；字段、认证标签、时间、请求或租户关联被替换，或格式合法但认证不匹配时一律返回 `False`——请求记录存在不能替代认证结果，请求或回执不存在同样只返回 `False`。旧密钥对轮换后新回执、新密钥对旧回执的认证不匹配同样返回 `False`。核验不写入、修复、回填、重算或覆盖任何回执。
- `rotate_receipt_key(tenant_id, retired_key, new_key)`：以“退用旧密钥 + 启用新密钥”原子轮换该租户的回执密钥，成功返回恰好 `generation`（轮换后的现用代次，正整数）与 `rotated_at`（UTC RFC3339 时间），仅在存储层开放、不经 HTTP。
  - 租户首次呈递密钥生成回执时登记为第 1 代；尚无现用代次时，首次轮换在同一事务登记第 1 代（退用密钥）与第 2 代（启用密钥，成为现用）；此后每次轮换追加一个新代次并把现用指针与新代次行在同一事务推进，未完成的轮换不会显示为已生效。
  - 轮换后新回执只接受现用密钥；旧代密钥不得生成新回执，旧回执不重写、不补发，但始终可用其原代次密钥核验。
  - 相同轮换（退用/启用组合与首次生效轮换一致）幂等返回首次代次与时间；密钥与当前代次不符、重复使用已被取代的轮换组合，或并发提交不同启用密钥时竞争失败，统一抛 `ReceiptKeyConflict` 且原代次不变（并发只产生一个现用代次，仅首个事务生效）。
  - 持久化只保存不可逆的密钥指纹、代次与轮换时间，密钥材料绝不进入数据库、结果、异常或日志。代次、轮换时间与历史回执在服务重启后仍可读取。
- 回执错误语义：生成入口的租户或密钥为空、类型或格式非法抛 `ValueError` 且不写库；请求编号为空、非字符串、非法或未知（含跨租户、尚未受理）统一抛 `RequestNotFound`，不区分记录是否存在、不泄露存在性；删除或执行记录未落定（未完成删除）抛 `ReceiptUnavailable`；用旧代或未知密钥生成新回执抛 `ReceiptKeyConflict`。轮换入口的租户、退用或启用密钥为空或类型非法、退用与启用密钥相同，抛 `ValueError` 且不提交变化；退用密钥与当前代次不符、缺少代次或重复使用已取代的轮换组合抛 `ReceiptKeyConflict`，原代次不变。核验入口参数非法抛 `ValueError`，文本格式或认证不匹配返回 `False`；数据库不可用、记录损坏或原子提交失败统一抛固定文案 `OSError`，不返回半成代次或半份回执。
