...
这是一个面向后端系统的机器遗忘证据服务，用于记录删除请求、执行状态和可验证回执。当前提供可运行的 Python 包、健康检查入口、删除请求的受理与查询 HTTP API（SQLite 持久化）、请求存储层上的可持久化状态机（状态推进与状态查询）、存储层上的删除执行编排（领取-租约-完成与执行记录）、存储层上的删除回执生成、核验与可恢复密钥轮换，以及存储层上的审计链外部可信锚点与全链核验（跨重启锚定、只读核验与恢复诊断）。状态推进、状态查询、执行编排、回执、密钥轮换与全链核验能力只在存储层开放，HTTP 仍只提供受理与受理回执查询两个端点，不新增任何执行、回执、密钥轮换或核验相关 HTTP 入口。
...
存储层审计链、外部可信锚点与全链核验（`RequestStore`，不经 HTTP 开放）：

- 每次受理和每次实际状态变化，都在同一事务内原子落定有序事件、请求链头与外部可信锚点：事件写入 `status_events` 并更新请求行链头的同时，追加一行 `chain_anchors` 锚点并推进全局锚点头；任一阶段提交失败统一抛固定文案 `OSError`，不返回成功、不留下可判完整的半成记录。幂等重放与同状态推进不产生事件，也不产生锚点。
- 锚点不能仅靠 SQLite 中的事件、链头或公开内容重算恢复：锚点是以库外持有的锚点密钥计算的 HMAC-SHA256，绑定全库锚点序号、租户、请求、事件序号、事件链哈希与前一锚点。锚点密钥由构造参数 `anchor_key`（非空字符串或字节）显式提供，或首次打开时生成并以仅属主权限写入数据库旁的侧车文件（`.<数据库文件名>.anchor-key`）；密钥材料、替代材料与链计算原文绝不写入库、回执、异常或日志。显式密钥与既有侧车不一致时抛固定文案 `OSError`。
- `verify_chain(tenant_id, request_id)` / `verify_evidence(tenant_id, request_id)`：只读全链核验，依次检查事件顺序（自零无缺口）、请求关联、请求链头与当前状态、每事件锚点与全局锚点序列、锚点头及跨重启结果。正常重建实例后同一审计链仍核验成功；重复查询与核验不改变任何记录。事件删改、插入、调序或跨请求、跨租户替换后返回 `False`；攻击者重算并替换库内事件、请求链头和公开锚点后，没有库外密钥也无法判伪造链有效。旧库无锚点、锚点损坏、链头不一致或提交中断时核验返回 `False`。
- `diagnose_chain(tenant_id, request_id)`：只读恢复诊断，返回恰好 `trusted`（布尔）与 `reason`（可信时为空值，否则为固定原因码：`event_chain_invalid`、`request_head_mismatch`、`anchors_missing`、`anchor_event_mismatch`、`anchor_sequence_broken`、`anchor_head_mismatch`、`anchor_authentication_failed`）。诊断只报告不可信原因，不修复、回填、重算或覆盖任何证据；结果只含安全业务字段。
- 审计与核验入口的空值、非字符串和非法参数抛 `ValueError` 且不改变证据；非法编号、未知请求或跨租户访问统一抛 `RequestNotFound`，不泄露存在性。既有数据库升级只回填事件链哈希（既有行为），绝不回填锚点：无锚点旧库核验恒为 `False`。
