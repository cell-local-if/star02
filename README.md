...
这是一个面向后端系统的机器遗忘证据服务，用于记录删除请求、执行状态和可验证回执。当前提供可运行的 Python 包、健康检查入口、删除请求的受理与查询 HTTP API（SQLite 持久化）、请求存储层上的可持久化状态机（状态推进与状态查询）、存储层上的删除执行编排（领取-租约-完成与执行记录），以及存储层上的外部可核验删除回执（生成与核验）。状态推进、状态查询、执行编排与删除回执只在存储层开放，HTTP 仍只提供受理与受理回执查询两个端点，不新增任何执行或回执相关 HTTP 入口。
...
- `generate_receipt(tenant_id, request_id, key)`：为**已完成删除且执行记录落定**的请求生成外部可核验回执。`tenant_id` 为非空字符串、`request_id` 为非空请求编号（不存在/跨租户/未受理统一 `RequestNotFound`），`key` 为调用方在数据库之外保管的非空字节串（非空 `bytes`/`bytearray`；空、缺失或类型非法抛 `ValueError` 且不写数据）。处理中（`accepted`/`processing`）或 `failed` 的请求不能取得删除回执，抛 `ReceiptUnavailable`。
  - 回执为紧凑单行 JSON（固定字段顺序、末尾恰好一个换行、UTC RFC3339 时间、64 位小写十六进制摘要），字段依次为 `receipt_version`、`receipt_type`、`tenant_id`、`request_id`、`accepted_at`（首次受理时间）、`completed_at`（最终完成时间）、`scope_commitment`（范围承诺摘要：受理时规范化范围集合的 SHA-256，不含原始范围）、`completion_commitment`（完成尝试摘要：最早完成尝试的序号、稳定终态类别与完成时间的长度前缀 SHA-256）、`auth_tag`（以调用方密钥对前八个字段计算的 HMAC-SHA-256，64 位小写十六进制）。
  - 存在多个终态尝试时，回执关联**最早完成**的执行结果（按完成时间、序号打破并列），后续重复终态不改变回执内容。
  - 回执只含业务字段与稳定错误类别，不暴露主体、原始范围、幂等键、worker、凭证、SQL 或路径；密钥材料不进入数据库、回执、异常或日志。
  - 回执行一次性原子写入：提交失败不留半份记录，也不改变执行终态或既有审计记录。同一请求重复生成返回字节一致的首次回执，并发生成只落一份（后到者取回首份），实例重建或服务重启后内容不变；已有回执绝不回填、重算或覆盖。回执记录损坏或存储不可用时查询抛固定文案 `OSError`。
- `verify_receipt(receipt, key)`：核验回执文本（`str`/`bytes`）与密钥。先严格解析（非文档形状、字段/顺序/编码/时间/摘要非法一律 `ValueError`），再以常量时间比较 HMAC 标签，最后与持久化的请求、范围承诺、最早完成尝试及首份回执逐字段比对。完整匹配返回 `True`；字段、认证标签、时间、请求或租户关联被替换均返回 `False`；格式合法但认证不匹配只返回 `False`，请求记录存在不能替代认证结果（未知/跨租户请求同样 `False`）。参数非法抛 `ValueError`，存储不可用或回执记录损坏抛固定文案 `OSError`；核验只读，绝不写入、修复或回填任何数据。
