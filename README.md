...
这是一个面向后端系统的机器遗忘证据服务，用于记录删除请求、执行状态和可验证回执。当前提供可运行的 Python 包、健康检查入口、删除请求的受理与查询 HTTP API（SQLite 持久化）、请求存储层上的可持久化状态机（状态推进与状态查询）、存储层上的删除执行编排（领取-租约-完成与执行记录）、存储层上的删除回执生成、核验与可恢复密钥轮换，以及存储层上的跨重启外部可信锚点、全链核验与可恢复的锚点密钥代次轮换（受理与每次实际状态变化的有序事件、请求链头、外部锚点与全局锚点原子落定，锚点按其生成时代次绑定密钥、只读全链核验与只报告原因的恢复诊断）。状态推进、状态查询、执行编排、回执、锚点核验、回执密钥轮换与锚点密钥轮换能力只在存储层开放，HTTP 仍只提供受理与受理回执查询两个端点，不新增任何执行、回执、密钥轮换或锚点相关 HTTP 入口。
