...
- 背景：仅靠库内事件哈希与请求链头，拿到数据库文件者可重算全部事件、链头并自洽，无法区分真实时间线与整条伪造链。本能力以调用方在数据库之外保管的锚点秘密为外部可信锚点：`RequestStore(db_path, anchor_secret=..., anchor_history_secrets=...)`。锚点秘密只存在于构造进程内存中，绝不入库、回执、返回值、异常或日志，库内只存不可逆的 HMAC 输出与不可逆密钥指纹；锚点也无法由库内事件、链头或任何公开内容重算恢复。`anchor_secret` 省略时维持历史无锚行为，但其全链核验结论为不可信。`anchor_history_secrets` 为可选的“代次→历史秘密”映射，仅在锚点密钥轮换后重建实例时用于核验旧代锚点；历史秘密同样只在内存、绝不入库。
...
- `diagnose_chain()` / `diagnose_audit_chain()`：只读恢复诊断，返回稳定、无细节的原因码列表（可信时为空），如 `unanchored_database`（旧库无锚点）、`anchor_secret_missing`（未持任何秘密核验已锚定库）、`anchor_key_missing`（持现用秘密但缺某历史代次秘密）、`anchor_auth_failed`（锚点不认证/密钥不符/代次关联被篡改或伪造）、`anchor_head_mismatch`（全局头不一致）、`chain_hash_mismatch`、`chain_head_mismatch`、`event_order_invalid`、`event_unanchored`、`anchor_orphan`、`anchor_state_split`、`anchor_sequence_gap`、`request_association_mismatch`、`request_status_mismatch`、`anchor_meta_corrupt`、`anchor_row_corrupt`。诊断**只报告**不可信原因，绝不修复、回填、重算或覆盖任何事件、链头或锚点。
...
- 锚点密钥代次轮换 `rotate_anchor_key(retired_secret, new_secret)`：可恢复的锚点密钥轮换，只在存储层开放，不经 HTTP、不新增健康命令。以“退用秘密 + 启用秘密”在单一事务原子切换代次，返回恰好两个字段：`generation`（正整数，新现用代次）与 `effective_at`（该代次生效的 UTC RFC3339 时间）。库内只保存代次、生效时间与不可逆指纹，秘密材料绝不入库、结果、异常或日志。
  - 尚无代次时，首次轮换在同一事务登记第 1 代（退用秘密）与第 2 代（启用秘密）并返回第 2 代；首个锚点本应在首次写时自举为第 1 代。轮换生效后新锚点只关联新现用代次；已有锚点继续关联各自生成时的代次，绝不追溯改写。
  - 相同轮换（与产生现用代次完全相同的组合）幂等返回首次代次与首次时间；相同组合的并发调用得到同一结果。
  - 并发提交不同启用秘密时只有首笔事务生效；竞争方（退用秘密为紧邻前一代、启用秘密未登记）抛 `AnchorKeyConflict`，代次不变。
  - 退用秘密不是现用（且不是紧邻前一代的竞争形态）、启用秘密已登记、秘密为空/非字符串/同值等参数非法时抛 `ValueError`，且不写库。
  - 重建实例时以 `anchor_secret` 提供当前秘密、以 `anchor_history_secrets` 提供相应历史代次秘密；旧代锚点与现用锚点分别在各自代次秘密下核验成功。缺某历史代次秘密时核验返回 `False` 且诊断报告 `anchor_key_missing`；历史秘密错配、代次关联被改或锚点伪造时返回 `False`，诊断为 `anchor_auth_failed`。数据库代次记录损坏、读写失败或轮换提交失败统一抛固定文案 `OSError`，不显示半成代次。
  - 服务重启后代次、生效时间与历史审计结论保持稳定，重复核验仍严格只读。
...
- `verify_receipt(receipt_text, key)`：核验回执文本与密钥。回执为紧凑 JSON 后接恰好一个末尾换行，末尾换行缺失或多余属于格式非法，抛 `ValueError`。仅当文本格式合法、与已落定回执记录逐字段（逐字节）完全一致、所给密钥属于该租户已登记的某一密钥代次、且认证标签在该密钥下重算一致时返回 `True`；字段、认证标签、时间、请求或租户关联被替换，或格式合法但认证不匹配时一律返回 `False`——请求记录存在不能替代认证结果，请求或回执不存在同样只返回 `False`。轮换后旧回执仍可用原（旧代）密钥验证；旧密钥对新回执、新密钥对旧回执、以及其他租户的密钥一律认证不匹配返回 `False`。核验不写入、修复、回填、重算或覆盖任何数据。
